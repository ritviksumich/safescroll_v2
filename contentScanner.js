...
(function () {
  ...
  const TEXT_SELECTORS  = 'h1, h2, h3, h4, h5, h6, p, li, blockquote, figcaption, td, th';
  ...
  function scanWithCache(classId, elementType, priority, payload, cacheKey) {
    ...
    if (window.ssPayloadCache[key] !== undefined) {
      ...
      return Promise.resolve(
        Object.assign({}, window.ssPayloadCache[key], { classID: classId })
      );
    }
    ...
    if (window.ssPayloadPending[key] !== undefined) {
      ...
      return window.ssPayloadPending[key].then(result =>
        Object.assign({}, result, { classID: classId })
      );
    }
  ...
  function buildTextEntry(el) {
    ...
    const payload     = (el.innerText || el.textContent || '').trim();
  ...
  function buildImageEntry(el) {
    ...
    return {
      ...
      action: function () {
        ...
        // img / picture: wait for load, then convert
        waitForImage(imgEl)
          .then(loaded => {
          ...
          .then(result => { entry.result = result; })
          .catch(() => {
            // broken or failed image — remove from queue
